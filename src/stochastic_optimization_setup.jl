...
function optimize_sp(pv, wind, demand, heatdemand, pars, n_samples, scen_freq;
    ...
    sp_bck=nothing,
    resample=false,
    resample_scens=nothing,
    opt="Cbc",
    cap_constraint="improved_heat",
)
    ...
    if maximum(heatdemand) == 0.0
        fix(decision_by_name(sp, 1, "u_heatpump"), 0.0)
        fix(decision_by_name(sp, 1, "u_heat_storage"), 0.0)
    end
    set_silent(sp)
    ...
    if resample
        if savefiles
            all_data = merge(
                get_all_data(sp, rec=false, scen=false),
                opt_params,
                Dict((
                    :runtime => runtime, 
